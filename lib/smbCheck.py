...
def check(ip, port=445):
    ...
    try:
        ...
        if nt_status == '\x05\x02\x00\xc0':
            return [True,"[+] [{}] is likely VULNERABLE to MS17-010! ({})".format(ip, native_os),"info"]
            ...
            if smb.multiplex_id == 0x0051:
              ...
              return [True,"Host is likely INFECTED with DoublePulsar! - XOR Key: {}".format(key),"info"]

        elif nt_status in ('\x08\x00\x00\xc0', '\x22\x00\x00\xc0'):
            return [False,"[-] [{}] does NOT appear vulnerable".format(ip),"error"]
        else:
            return [False,"[-] [{}] Unable to detect if this host is vulnerable".format(ip),"error"]

    except Exception as err:
        return [False,"[-] [{}] Exception: {}".format(ip, err),"error"]
    finally:
        client.close()
